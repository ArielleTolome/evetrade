 ...
 * @param {number} [props._stationId] - Station ID (currently unused but reserved for future features)
...
export function EveActionButtons({
  ...
  const handleOpenMarket = () => {
    try {
      ...
      toast.success('Opening market details in EVE...');
    } catch (error) {
      toast.error(error.message || 'Failed to open EVE client');
    }
  };
  ...
  const handleShowInfo = () => {
    try {
      showItemInfo(typeId);
      toast.success('Opening item info in EVE...');
    } catch (error) {
      toast.error(error.message || 'Failed to open EVE client');
    }
  };
  ...
  const handleSetDestination = () => {
    if (!solarSystemId) {
      toast.warning('No destination available for this item');
      return;
    }

    try {
      setDestination(solarSystemId);
      toast.success('Destination set in EVE...');
    } catch (error) {
      toast.error(error.message || 'Failed to set destination');
    }
  };

  const handleCopyName = () => {
    if (!typeName) {
      toast.warning('No item name to copy');
      return;
    }
    ...
    copyToClipboardWithFeedback(
      typeName,
      () => {
        toast.success(`Copied "${typeName}" to clipboard`);
